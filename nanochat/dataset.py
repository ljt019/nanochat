...
def list_parquet_files_finewiki(data_dir=None):
    data_dir = FINEWIKI_DATA_DIR if data_dir is None else data_dir
    parquet_files = sorted(
        [
            f
            for f in os.listdir(data_dir)
...
def parquets_iter_batched_finewiki(split, start=0, step=1, data_dir=None):
    assert split in ["train", "val"], "split must be 'train' or 'val'"
    parquet_paths = list_parquet_files_finewiki(data_dir=data_dir)
    # No explicit val shard in finewiki release, so just use same logic
    parquet_paths = parquet_paths[:-1] if split == "train" else parquet_paths[-1:]
    for filepath in parquet_paths:
        pf = pq.ParquetFile(filepath)
        for rg_idx in range(start, pf.num_row_groups, step):
...
def _download_dataset(dataset, num_files, num_workers):
    if dataset == "fineweb":
        max_shard = MAX_SHARD
        data_dir = DATA_DIR
        download_fn = download_single_file
    elif dataset == "finewiki":
        max_shard = FINEWIKI_MAX_SHARD
        data_dir = FINEWIKI_DATA_DIR
        download_fn = download_single_file_finewiki
    else:
...
def download_single_file_finewiki(index):
    filename = finewiki_index_to_filename(index)
    filepath = os.path.join(FINEWIKI_DATA_DIR, filename)
    if os.path.exists(filepath):
        print(f"Skipping {filepath} (already exists)")
        return True

    url = f"{FINEWIKI_BASE_URL}/{filename}"
    print(f"Downloading {filename}...")
...
if __name__ == "__main__":
    ...
    _download_dataset(args.dataset, args.num_files, args.num_workers)
