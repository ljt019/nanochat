...
try:
    model, tokenizer, meta = load_model(
        "knowledge", device, phase="train", model_tag=model_tag, step=step
    )
    print0("Loaded knowledge midtraining checkpoint")
except FileNotFoundError:
    print0("Knowledge checkpoint not found, falling back to base checkpoint")
    model, tokenizer, meta = load_model(
